...
XMFLOAT3 CHeightMapImage::GetHeightMapNormal(int x, int z)
{
	...
	XMFLOAT3 xmf3Normal = Vector3::CrossProduct(xmf3Edge1, xmf3Edge2, false);
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT3 xmf3Normal, void* pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			m_pxmf3Normal[i] = XMFLOAT3(Vector3::Add(m_pxmf3Positions[i], pHeightMapImage->GetHeightMapNormal(x, z)));
			m_pxmf2TextureCoords0[i] = XMFLOAT2(float(x) / float(cxHeightMap - 1), float(czHeightMap - 1 - z) / float(czHeightMap - 1));
			m_pxmf2TextureCoords1[i] = XMFLOAT2(float(x) / float(m_xmf3Scale.x * 0.8f), float(z) / float(m_xmf3Scale.z * 0.8f));
...
void CHeightMapGridMesh::Render(ID3D12GraphicsCommandList* pd3dCommandList, int nSubSet)
{
	...
	D3D12_VERTEX_BUFFER_VIEW pVertexBufferViews[4] = { m_d3dPositionBufferView, m_d3dNormalBufferView, m_d3dTextureCoord0BufferView, m_d3dTextureCoord1BufferView };
...
void CSphereMeshIlluminated::Render(ID3D12GraphicsCommandList* pd3dCommandList, int nPipelineState)
{
	CMesh::Render(pd3dCommandList, nPipelineState);
}
